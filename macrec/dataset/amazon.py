...
def process_interaction_data_with_retrieval(data_df: pd.DataFrame, n_retrieval_items: int = 20) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    # Note: For Amazon datasets, embeddings directory would be dataset-specific
    # This is a placeholder - actual implementation would need to determine the correct embeddings path
    # based on the dataset category (Beauty, Electronics, etc.)
    logger.info("Note: Amazon dataset embedding mappings should be created manually or with dataset-specific paths")
    ...
    out_df = retrieval_sample(out_df)
...
def process_data_with_retrieval(dir: str, n_retrieval_items: int = 20):
    """Process the amazon raw data using retrieval-based sampling and output the processed data to `dir`.
    ...
    train_df, dev_df, test_df, out_df = process_interaction_data_with_retrieval(data_df, n_retrieval_items)
    logger.info(f'Number of interactions: {out_df.shape[0]}')
    ...
    dfs = append_his_info([train_df, dev_df, test_df], summary=True, neg=False)  # neg=False since we don't have neg_item_id
    logger.info('Completed append history information to interactions')
    for df in dfs:
        # format history by list the historical item attributes
        df['history'] = df['history_item_id'].apply(lambda x: item_df.loc[x]['item_attributes'].values.tolist())
