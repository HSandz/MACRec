...
from macrec.dataset import ml100k_process_data, amazon_process_data, ml1m_process_data, ml100k_process_data_with_retrieval, amazon_process_data_with_retrieval, ml1m_process_data_with_retrieval
...
class PreprocessTask(Task):
    ...
    def parse_task_args(parser: ArgumentParser) -> ArgumentParser:
        ...
        parser.add_argument('--dataset', type=str, required=True, choices=['ml-100k', 'ml-1m', 'amazon'], help='dataset type')
    ...
    def run(self, data_dir: str, dataset: str, amazon_category: str = None, n_neg_items: int = 7, sampling_strategy: str = 'negative', n_retrieval_items: int = 20):
        ...
        if dataset == 'ml-100k':
            if sampling_strategy == 'retrieval':
                ml100k_process_data_with_retrieval(data_dir, n_retrieval_items)
            else:
                ml100k_process_data(data_dir, n_neg_items)
        elif dataset == 'ml-1m':
            if sampling_strategy == 'retrieval':
                ml1m_process_data_with_retrieval(data_dir, n_retrieval_items)
            else:
